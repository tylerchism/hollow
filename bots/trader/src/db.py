...
-- V1: RSI signals generated by the strategy engine
CREATE TABLE IF NOT EXISTS signals (
    ...
    rsi         REAL,
    close_price REAL,
    candle_ts   TEXT NOT NULL,          -- timestamp of the candle that triggered the signal
    acted_on    INTEGER DEFAULT 0,      -- 1 if the engine placed/skipped an order on it
    expired     INTEGER DEFAULT 0,      -- 1 if the signal aged out before acting
    notes       TEXT
);

-- V1: Paper trades — append-only, written BEFORE simulator executes the fill
CREATE TABLE IF NOT EXISTS paper_trades (
    ...
    qty             REAL NOT NULL,
    intent_price    REAL NOT NULL,          -- price at signal time
    fill_price      REAL,                   -- actual fill after slippage
    slippage_pct    REAL,
    fill_value_usd  REAL,
    status          TEXT NOT NULL DEFAULT 'pending',  -- 'pending' | 'filled' | 'rejected'
    signal_id       INTEGER REFERENCES signals(id),
    position_id     TEXT,                   -- UUID linking open/close legs
    notes           TEXT
);

-- V1: Open paper positions — one row per open position
CREATE TABLE IF NOT EXISTS paper_positions (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    position_id     TEXT NOT NULL UNIQUE,   -- UUID
    strategy_id     TEXT NOT NULL,
    symbol          TEXT NOT NULL,
    side            TEXT NOT NULL,          -- 'long' | 'short' (V1 only uses 'long')
    qty             REAL NOT NULL,
...
-- V1: Risk state — single-row table tracking current risk flags
CREATE TABLE IF NOT EXISTS risk_state (
    id                      INTEGER PRIMARY KEY CHECK (id = 1),
    killed                  INTEGER DEFAULT 0,      -- kill switch engaged
    killed_at               TEXT,
    killed_reason           TEXT,
    daily_drawdown_halted   INTEGER DEFAULT 0,
    halt_date               TEXT,
    day_start_value_usd     REAL,
